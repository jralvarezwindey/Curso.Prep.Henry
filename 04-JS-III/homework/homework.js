...
function mesesDelAño(array) {
  ...
  for (var i = 0; i < array.length; i++) {
    switch (array[i]) {
      ...
      break;
      case 'Noviembre':
      tresMeses[1] = 'Noviembre';
    } 
  }
  if (tresMeses[0] === 0 || tresMeses[1] === 0 || tresMeses[2] === 0) return 'No se encontraron los meses pedidos';
  return tresMeses;
}
...
function continueStatement(numero) {
  ...
  // Tu código:
  var numeroArray=[numero+2];
  for (var i = 1; i < 10; i++) {
    if (i < 4) {
      numeroArray.push(numeroArray[i-1]+2);
    }
    else if (i > 4){
      numeroArray.push(numeroArray[i-2]+2);
    }
  }
  return numeroArray;
}
